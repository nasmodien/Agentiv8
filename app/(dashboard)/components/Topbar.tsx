  ...
  useEffect(() => {
    fetch('/api/properties?orgId=default')
      ...
      .then(data => {
        ...
        // Compute stats from properties + bookings
        ...
        // Escalations from conversations
        fetch('/api/messages').then(r => r.json()).then(d => {
          escalations = (d.conversations ?? []).filter((c: { status: string }) => c.status === 'ESCALATED').length;
          setStats({ totalProperties: props.length, bookedProperties: booked, checkInsToday: checkIns, escalations });
        }).catch(() => {
          setStats({ totalProperties: props.length, bookedProperties: booked, checkInsToday: checkIns, escalations: 0 });
        });
      })
      .catch(() => {});
